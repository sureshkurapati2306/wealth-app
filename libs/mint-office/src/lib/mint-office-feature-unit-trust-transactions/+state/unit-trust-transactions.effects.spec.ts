...
import { UnitTrustTransactionsEffects } from './unit-trust-transactions.effects';
...
import { State } from './unit-trust-transactions.reducer';
...
describe('UnitTrustTransactionsEffects', () => {
  ...
  it('should run loadUnitTrustTransactions Success path', (done) => {
    ...
    actions$ = of(Actions.loadUnitTrustTransactions({
      searchParams: {
        status: 'All'
      }
    }));
    ...
    effects.loadUnitTrustTransactions$
      .subscribe(action => {
        expect(action).toEqual(Actions.loadUnitTrustTransactionsSuccess({
          data: mockData
        }));
  ...
  it('should run cancelUnitTrustTransactions Success path', (done) => {
      
    const spy = jest.spyOn(unitTrustTransactionsService, 'cancelUnitTrustTransactions').mockReturnValue(of(mockData));
    
    // create an actions stream and immediately dispatch a GET action
    actions$ = of(Actions.cancelUnitTrustTransactions({
      payload: []
    }));

    // subscribe to the Effect stream and verify it dispatches a SUCCESS action
    effects.cancelUnitTrustTransactions$
      .subscribe(action => {
        expect(action).toEqual(Actions.cancelUnitTrustTransactionsSuccess({
          data: mockData
        }));
        expect(spy).toHaveBeenCalledTimes(1);
        done();
  ...
  it('should run cancelUnitTrustTransactions Failure path #1', (done) => {
      
    const spy = jest.spyOn(unitTrustTransactionsService, 'cancelUnitTrustTransactions').mockReturnValue(throwError('Mock error'));
    ...
    effects.cancelUnitTrustTransactions$
      .subscribe(action => {
        expect(action).toEqual(Actions.cancelUnitTrustTransactionsFailure({
          error: 'Fail to cancel UT Transaction!'
        }));
        expect(spy).toHaveBeenCalledTimes(1);
        done();
      });
  ...
  it('should run cancelUnitTrustTransactions Failure path #2', (done) => {
      
    const spy = jest.spyOn(unitTrustTransactionsService, 'cancelUnitTrustTransactions').mockReturnValue(throwError(
      {
        error: {
          details: 'Mock error'
        }
